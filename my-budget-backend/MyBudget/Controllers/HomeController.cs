...
namespace MyBudget.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.totalGider = String.Format("{0:0.00}", db.Transaction.Where(x => x.UserId == CurrentSession.User.Id).Sum(x => x.Amount));
            ViewBag.totalThisMonthGider = String.Format("{0:0.00}", db.Transaction.Where(x => x.UserId == CurrentSession.User.Id && x.CreatedDate.Value.Month == currentDate.Month && x.CreatedDate.Value.Year == currentDate.Year).Sum(x => x.Amount));
